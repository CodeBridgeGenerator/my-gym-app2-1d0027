...
import membershipplansFakerFactory from "./membershipplansFakerFactory";
...
const MembershipplansFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = membershipplansFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("membershipplans").create(elem));
...
export default connect(mapState, mapDispatch)(MembershipplansFakerDialogComponent);
