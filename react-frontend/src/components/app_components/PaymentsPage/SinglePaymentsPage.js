    ...
    useEffect(() => {
        ...
        client
            ...
            : res.userId
                ...
                : [];
        setUserId(userId);
const planId = Array.isArray(res.planId)
            ? res.planId.map((elem) => ({ _id: elem._id, planName: elem.planName }))
            : res.planId
                ? [{ _id: res.planId._id, planName: res.planId.planName }]
                : [];
        setPlanId(planId);
            })
            .catch((error) => {
    ...
    return (
        ...
        <div className="col-12 flex flex-column align-items-center">
            ...
            <div className="col-12 md:col-6 lg:col-3"><label className="text-sm text-gray-600">User Id</label>
                    {userId.map((elem) => (
                        <Link key={elem._id} to={`/members/${elem._id}`}>
                        <div>
                  {" "}
...
<div className="col-12 md:col-6 lg:col-3"><label className="text-sm text-gray-600">Plan Id</label>
                    {planId.map((elem) => (
                        <Link key={elem._id} to={`/membershipplans/${elem._id}`}>
                        <div>
                  {" "}
                            <p className="text-xl text-primary">{elem.planName}</p>
                            </div>
                        </Link>
                    ))}</div>
