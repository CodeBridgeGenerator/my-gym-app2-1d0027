...
const [planIdIds, setplanIdIds] = useState();
...
client.service("membershipplans")
                        .find({ query: { $select: ["_id","planName"] } })
                        .then((data) => {
                        setplanIdIds(data.data);
                        });
    ...
    const onRun = async () => {
        let fakeData = paymentsFakerFactory(props.user,fakerCount,userIdIds,planIdIds);
