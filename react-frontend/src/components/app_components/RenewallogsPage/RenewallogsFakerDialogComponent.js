...
import renewallogsFakerFactory from "./renewallogsFakerFactory";
...
const RenewallogsFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = renewallogsFakerFactory(props.user,fakerCount,userIDIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("renewallogs").create(elem));
...
export default connect(mapState, mapDispatch)(RenewallogsFakerDialogComponent);
